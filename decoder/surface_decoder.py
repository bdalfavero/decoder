...
def is_error_logical_bit_flip(d: int, err: cirq.PauliString) -> bool:
    ...
    # Start with a graph containing all data and measure qubits.
    qubit_graph = nx.Graph()
    for qubit in qs:
        qubit_graph.add_node(qubit)
    for i in range(qubits_per_side): # Iterate over rows
        for j in range(qubits_per_side): # Iterate over cols
            if i != 0: # North neighbor
                qubit_graph.add_edge(cirq.GridQubit(i, j), cirq.GridQubit(i-1, j))
            if i != qubits_per_side - 1: # South neighbor
                qubit_graph.add_edge(cirq.GridQubit(i, j), cirq.GridQubit(i+1, j))
            if j != 0: # West neighbor
                qubit_graph.add_edge(cirq.GridQubit(i, j), cirq.GridQubit(i, j-1))
            if j != qubits_per_side - 1: # East neighbor
                qubit_graph.add_edge(cirq.GridQubit(i, j), cirq.GridQubit(i, j+1))
    # Then take out the qubits that are not in the support of the X,Y portion of the error.
    # This means paths from the top to the bottom must go through error qubits.
    for qubit in qs:
        is_data_qubit = ((qubit.row == 0) and (qubit.col == 0))\
            or ((qubit.row != 0) and (qubit.col != 0))
        if qubit not in xy_err.keys() and is_data_qubit:
            qubit_graph.remove_node(qubit)
    # Our "default" logical X runs along the left (ragged) edge of the lattice.
    # See fig. 8 of Chubb and Flammia or fig. 1 of O'Rourke and Devitt 2024.
    # See if there are any paths between a top edge node and a bottom edge node.
    top_qubits = cirq.GridQubit.rect(1, qubits_per_side)
    bottom_qubits = cirq.GridQubit.rect(1, qubits_per_side, top=qubits_per_side-1)
    path_exists = False
    for tq in top_qubits:
        for bq in bottom_qubits:
            if tq in qubit_graph.nodes and bq in qubit_graph.nodes:
                all_paths = list(nx.all_simple_paths(qubit_graph, tq, bq))
                if len(all_paths) != 0:
                    path_exists = True
    return path_exists
